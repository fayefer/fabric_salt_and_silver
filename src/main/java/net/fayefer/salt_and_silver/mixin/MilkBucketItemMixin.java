package net.fayefer.salt_and_silver.mixin;
...
public abstract class MilkBucketItemMixin extends Item implements FluidModificationItem {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (ModFluids.MILK_FLUID_STILL == null || (blockHitResult.getType() == HitResult.Type.MISS || user.isSneaking())) {
	...
	public boolean placeFluid(@Nullable PlayerEntity player, World world, BlockPos pos, @Nullable BlockHitResult hitResult) {
		if (ModFluids.MILK_FLUID_STILL == null)
			return false;
		BlockState blockState = world.getBlockState(pos);
		Block block = blockState.getBlock();
		boolean canPlace = blockState.canBucketPlace(ModFluids.MILK_FLUID_STILL);
		boolean bl2 = blockState.isAir() ||
				canPlace ||
				block instanceof FluidFillable fillable && fillable.canFillWithFluid(player, world, pos, blockState, ModFluids.MILK_FLUID_STILL);
		if (!bl2) {
			...
			fillable.tryFillWithFluid(world, pos, blockState, ModFluids.MILK_FLUID_FLOWING.getStill(false));
			...
			if (!world.setBlockState(pos, ModFluids.MILK_FLUID_STILL.getDefaultState().getBlockState(), Block.NOTIFY_ALL | Block.REDRAW_ON_MAIN_THREAD)
